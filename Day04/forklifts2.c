...
//		printf("Line: %d: %s", lineCount, lineBuf);
	...
	//store removed rolls
	uint8_t	*pWHDataRR	=malloc(lineLen * lineCount);
	memset(pWHDataRR, 0, lineLen * lineCount);
...
*/
	...
	for(;;)
	{
		...
		//first mark the rolls to be moved
		for(int y=0;y < lineCount;y++)
		{
			for(int x=0;x < lineLen;x++)
			{
		...
		//remove previous x's to make the prints look better
		for(int y=0;y < lineCount;y++)
		{
			for(int x=0;x < lineLen;x++)
			{
